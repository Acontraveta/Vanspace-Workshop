...
export default function QuotesList({ onEditQuote }: QuotesListProps) {
  ...
  const handleCancel = (quoteId: string) => {
    confirm('¿Cancelar este presupuesto?', async () => {
      try {
  ...
  const handleDelete = (quoteId: string) => {
    confirm('¿Eliminar este presupuesto? Esta acción no se puede deshacer.', async () => {
      try {
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge variant={
                  ...
                  {{ DRAFT: '📝 Borrador', SENT: '📤 Enviado', APPROVED: '✅ Aprobado', REJECTED: '❌ Cancelado', EXPIRED: '⏰ Caducado' }[quote.status] ?? quote.status}
                </Badge>
                {isExpiringSoon && (
