...
export default function ExteriorDesignList() {
  ...
  const load = async () => {
    try {
      setLoading(true)
      const wo = await FurnitureWorkOrderService.getAllByType('exterior')
      setOrders(wo)
    } catch (err: any) {
      toast.error('Error cargando órdenes de diseño exterior')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Órdenes de diseño exterior generadas desde presupuestos
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => navigate('/design/exterior/free')}
            className="px-5 py-2.5 bg-blue-600 text-white text-xs font-black uppercase rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all"
          >
            + Diseño libre
          </button>
        </div>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <h3 className="text-lg font-bold text-slate-700">Sin órdenes de diseño exterior</h3>
          <p className="text-sm text-slate-400 mt-1">
            Aparecerán aquí cuando se aprueben presupuestos con elementos exteriores (ventanas, claraboyas, placas solares…)
          </p>
          <button
            onClick={() => navigate('/design/exterior/free')}
            className="mt-4 px-5 py-2.5 bg-blue-600 text-white text-xs font-black uppercase rounded-xl hover:bg-blue-700 transition-all"
          >
            + Crear diseño libre
          </button>
        </div>
        ...
        <div className="space-y-3">
          {orders.map(wo => {
            ...
            return (
              <div
                key={wo.id}
                onClick={() => navigate(`/design/exterior/order/${wo.id}`)}
                className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 shadow-sm hover:shadow-md hover:border-blue-300 cursor-pointer transition-all"
              >
                    ...
                    <div className="flex-1 max-w-[200px] h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          designedCount === items.length ? 'bg-green-500' : 'bg-blue-500'
                        }`}
